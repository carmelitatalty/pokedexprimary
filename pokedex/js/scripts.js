...
let pokemonUI = ( function() {
  ...
  function loadUpdatedMenu() {
    ...
    const clickPokemonPage = function( event ) {
      pokemonApp.loadPokemon( event.data.pageNumber, limit );
    };
    ...
    while ( offset < count ) {
      ...
      let li = $( `<li class="btn btn-light page-item page-link"
       id="paging-${pageNumber}">${pageNumber++}</li>` );
      pagingList.append( li );

      li.on( 'click', { pageNumber: pNumber }, clickPokemonPage );
  ...
  function addListItem( pokemon ) {
    ...
    let pokeballButton = $(
      `<button type="button" class="img-fluid poke-button btn btn-link" data-toggle="modal"
       data-target="#exampleModal" data-pokemon-name="${pokemon.name}"
        data-pokemon-url="${pokemon.detailsUrl}"></button>`
    );
